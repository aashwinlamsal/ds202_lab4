...
cleanOffense<- offense %>% mutate_at(c("Receiving_REC","Receiving_YDS","Receiving_TD","Rushing_ATT","Rushing_YDS", "Rushing_TD", "Passing_CMP-ATT", "Passing_YDS", "Passing_TD", "Passing_INT"), funs(as.numeric), na.rm=TRUE)

str(cleanOffense) #checking if changes were made successfully

cleanDefense<- defense %>% mutate_at(c("Tackles_Solo", "Tackles_ASST", "Tackles_TFL","Tackles_Sack","Turnover_FF", "Turnover_FR", "Turnover_INT", "Pass_QBH", "Pass_PB"), funs(as.numeric), na.rm=TRUE)

str(cleanDefense)#checking if changes were made successfully
...
bio_split = strsplit(as.character(bioHeight$Height), split = "-")
bioHeight = transform(bioHeight, Height_Feet = sapply(bio_split, "[[", 1), Height_Inches = sapply(bio_split, "[[", 2))

bioHeight = bioHeight %>% mutate_at(c("Height_Feet", "Height_Inches"), funs(as.numeric))

bioHeight$Height_Feet_Inches = bioHeight$Height_Feet * 12

bioHeight$Height_Inches = bioHeight$Height_Inches + bioHeight$Height_Feet_Inches



bioClean = bioHeight %>% select(-c("Height","Height_Feet", "Height_Feet_Inches", "Hometown", "Highschool"))

...
defWV = defClean %>% filter(defClean$Opponent_Opponent == "West Virginia" & defClean$Statistic == "Tackles_Solo")

defWV %>% group_by(defWV$Name) %>% summarise(Count = n())

defKState = defClean %>% filter(defClean$Opponent_Opponent == "Kansas State")


ggplot(defClean, aes(x = defWV$Tackles_Solo, y = defKState$)) + geom_point()
